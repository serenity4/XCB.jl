mutable struct Window
    ...
    function Window(conn, id, parent_id, visual_id, value_masks, value_list; depth=XCB_COPY_FROM_PARENT, x=0, y=0, width=512, height=512, border_width=1, class=XCB_WINDOW_CLASS_INPUT_OUTPUT, window_title="", icon_title=nothing)
        ...
        win = new(conn, id, parent_id, visual_id, class, depth, value_masks, value_list, window_title, icon_title)
        xcb_create_window(
            win.conn.h,
            depth,
            win.id,
...
Window(conn, id, screen, value_masks, value_list; kwargs...) = Window(conn, id, screen.root, screen.root_visual, value_masks, value_list; kwargs...)
